...
#We checked the data for duplicates and found two in hhid, so we removed them.
...
data_clean_hh <- data_tidy_hh %>%
    ...
    #Clean the crop other variable (capitalize first letetr)
    mutate(crop_ther = str_to_title(crop_other)) %>% 
...
data_tidy_mem <- data_dedup %>%
    select(vid:enid, starts_with("gender"),starts_with("age"),starts_with("read"),
